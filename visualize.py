...
model = word2vec.load('1000v500.bin')
...
index2,metrics2 = model.cosine(u'贯彻落实')
index3,metrics3 = model.cosine(u'红十字')
index4,metrics4 = model.cosine(u'自动化')
index5,metrics5 = model.cosine(u'农民工')

# add the index of center word 
index01=np.where(model.vocab==u'中国')
index02=np.where(model.vocab==u'贯彻落实')
index03=np.where(model.vocab==u'红十字')
index04=np.where(model.vocab==u'自动化')
index05=np.where(model.vocab==u'农民工')
...
# plot the result
zhfont = matplotlib.font_manager.FontProperties(fname='Font.ttf')
fig = plt.figure()
